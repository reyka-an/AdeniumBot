...
public async Task OnSlashCommandAsync(SocketSlashCommand command)
{
    ...
    try
    {
        ...
        {
            await command.FollowupAsync("Нужно указать /quest done number:<число> user:<пользователь>.", ephemeral: true);
            return;
        }
    ...
    catch (Exception ex)
    {
        ...
        try { await command.FollowupAsync("Произошла ошибка при обработке команды.", ephemeral: true); }
        catch { /* уже поздно отвечать — ок */ }
    }
}
